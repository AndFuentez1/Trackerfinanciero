...
import { CashFlowPoint } from '@/features/finance/cashflow/components/cashflow.types';
...
export function useCashFlow(year: number, month: number | 'all', range: 'mes' | '6m' | 'año', incomeMode: 'real' | 'projected' = 'real') {
  ...
  const { data: futureExpenses = [] } = useQuery({
    ...
    placeholderData: (previousData) => previousData, // User Requirement: Keep previous data
  });

  // Realtime subscription for future expenses
  useEffect(() => {
    if (!user) return;
  ...
  function calculateMonthlySnapshot(date: Date, prevBalance: number, prevSavings: Record<string, number>, prevLoans: Record<string, { saldo: number, cuotasRestantes: number }>, prevCardInstallments: Record<string, { restantes: number, valorCuota: number, interes: number, capital: number }>) {
    ...
    // Helper to identify Salary category
    const isSalary = (catId: string | null, catName?: string) => {
      if (catName && catName.toLowerCase().includes('salario')) return true;
      if (!catId) return false;
      const cat = categories.find(c => c.id === catId);
      return cat ? cat.name.toLowerCase().includes('salario') : false;
    };
    ...
    transactions.forEach(tx => {
      ...
      if (isSameMonth && !isMultiCuotaCC) {
        if (tx.type === 'expense') {
          ...
          if (isSalary(tx.category_id, tx.category)) {
    ...
    budgets.forEach(b => {
      ...
      if (cat && cat.type === 'income') {
        if (cat.name.toLowerCase().includes('salario')) {
          budgetedSalary += b.budget.amount;
        } else {
          budgetedOther += b.budget.amount;
        }
      }
    });
    ...
    if (incomeMode === 'projected' && isFutureRelativeToIncome) {
      ingresosSalario = budgetedSalary;
      otrosIngresos = budgetedOther;
    } else {
  ...
  const cashFlowSeries = monthlyBreakdown.map((row, idx) => {
    ...
    return {
      ...
      nameMobile: format(addMonths(start, idx), 'MMMMM', { locale: es }).charAt(0).toUpperCase(), // Single letter for mobile
