...
import { PaymentMethodRow, TransactionRow, TransactionType } from './useFinanceData';
...
export function useSavingsData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Fetch payment methods that are savings accounts or have specific types
    const { data: accountsData, error: accountsError } = await supabase
      .from('payment_methods')
      .select('*')
      .eq('user_id', user.id)
      .or(`is_savings_account.eq.true,type.in.("savings","investment")`);
  ...
  const addSavingsAccount = async (account: { name: string; balance?: number; interest_rate?: number }) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        estimated_yield: account.interest_rate || 0,
