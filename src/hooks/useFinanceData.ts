...
export function useFinanceDataLogic() {
  ...
  const fetchTransactions = useCallback(async (isLoadMore = false): Promise<{ data: Transaction[], total: number }> => {
    ...
    // 2. Fetch ALL records for the range to power charts/summaries (only if not loading more, to avoid redundant full fetches)
    let rangeQuery = null;
    if (!isLoadMore) {
      rangeQuery = supabase
        .from('transactions')
  ...
  const fetchData = useCallback(async () => {
    ...
    if (categoriesRes.error) {
      ...
      if (categoriesToUpdate.length > 0) {
        console.log('Fixing legacy/missing category colors:', categoriesToUpdate);
  ...
  const convertCurrency = async (rate: number, newCurrency: string, dryRun = false) => {
    ...
    try {
      ...
      console.log('Objeto a enviar a Supabase (payment_methods):', pmUpdates);
      console.log('Objeto a enviar a Supabase (transactions):', txUpdates);
  ...
  const addCategory = async (category: Omit<CategoryItem, 'id' | 'created_at'>) => {
    try {
      ...
      // Prevention of Duplicates in Categories
      if (categories.some(c => c.name.toLowerCase() === finalName.toLowerCase())) {
        toast({
          title: 'Aviso',
          description: 'Esta categoría ya existe',
          variant: 'default'
        });
        return { error: 'Categoría existente' };
      }
