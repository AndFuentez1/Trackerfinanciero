...
/** Theme var "H S% L%" to hex */
function themeVarToHex(varValue: string): string {
  const { h, s, l } = parseHSLString(varValue);
  return hslToHex(h, s, l);
}

/** Find L (0-100) for neutral color (h=0,s=0) that gives contrast >= minRatio on backgroundHex */
function findLForContrast(backgroundHex: string, minRatio: number, preferDark: boolean): number {
  ...
  if (preferDark) {
    for (let L = 10; L <= 50; L += 2) {
      const hex = hslToHex(0, 0, L);
      const { ratio } = getContrastRatio(hex, backgroundHex);
      if (ratio >= minRatio) return L;
    }
    return 15;
  } else {
    for (let L = 99; L >= 50; L -= 2) {
      const hex = hslToHex(0, 0, L);
      const { ratio } = getContrastRatio(hex, backgroundHex);
      if (ratio >= minRatio) return L;
    }
...
/**
 * Validate contrast for a theme color and computed theme pairs.
 * Also validates primary-foreground on primary and muted-foreground on background.
 * If contrast < 4.5:1, returns adjusted HSL values for WCAG 2.1 AA.
 */
export function validateThemeContrast(baseColorHex: string): {
  ...
  primaryForegroundContrast: { ratio: number; level: 'AAA' | 'AA' | 'FAIL' };
  mutedForegroundContrast: { ratio: number; level: 'AAA' | 'AA' | 'FAIL' };
  isAccessible: boolean;
  adjustedPrimaryForeground?: string;
  adjustedMutedForeground?: string;
} {
  ...
  const theme = calculateProportionalTheme(baseColorHex);
  const primaryVar = theme['--primary'] ?? theme['--accent-primary'] ?? '220 50% 50%';
  const primaryForegroundVar = theme['--primary-foreground'] ?? '0 0% 100%';
  const backgroundVar = theme['--background'] ?? '0 0% 100%';
  const mutedForegroundVar = theme['--muted-foreground'] ?? '220 10% 45%';

  const primaryHex = themeVarToHex(primaryVar);
  const primaryForegroundHex = themeVarToHex(primaryForegroundVar);
  const backgroundHex = themeVarToHex(backgroundVar);
  const mutedForegroundHex = themeVarToHex(mutedForegroundVar);

  const primaryForegroundContrast = getContrastRatio(primaryForegroundHex, primaryHex);
  const mutedForegroundContrast = getContrastRatio(mutedForegroundHex, backgroundHex);

  let adjustedPrimaryForeground: string | undefined;
  let adjustedMutedForeground: string | undefined;

  if (primaryForegroundContrast.ratio < WCAG_AA_RATIO) {
    const primaryLum = getLuminance(
      parseInt(primaryHex.slice(1, 3), 16),
      parseInt(primaryHex.slice(3, 5), 16),
      parseInt(primaryHex.slice(5, 7), 16)
    );
    const L = findLForContrast(primaryHex, WCAG_AA_RATIO, primaryLum > 0.5);
    adjustedPrimaryForeground = `0 0% ${L}%`;
  }

  if (mutedForegroundContrast.ratio < WCAG_AA_RATIO) {
    const { h, s } = parseHSLString(mutedForegroundVar);
    for (let L = 15; L <= 85; L += 3) {
      const fgHex = hslToHex(h, s, L);
      const { ratio } = getContrastRatio(fgHex, backgroundHex);
      if (ratio >= WCAG_AA_RATIO) {
        adjustedMutedForeground = `${h} ${s}% ${L}%`;
        break;
      }
    }
    if (!adjustedMutedForeground) adjustedMutedForeground = `${h} ${s}% 25%`;
  }

  const isAccessible =
    fgContrast.level !== 'FAIL' &&
    bgContrast.level !== 'FAIL' &&
    primaryForegroundContrast.ratio >= WCAG_AA_RATIO &&
    mutedForegroundContrast.ratio >= WCAG_AA_RATIO;

  return {
    foregroundContrast: fgContrast,
    borderContrast: bgContrast,
    primaryForegroundContrast,
    mutedForegroundContrast,
    isAccessible,
    adjustedPrimaryForeground,
    adjustedMutedForeground,
  };
}
...
export function useFinanceDataLogic() {
    ...
    try {
      ...
      for (const table of tables) {
        const { error } = await supabase
          ...
          .eq('user_id', user.id);
        if (error) {
          errors.push(`${String(table)}: ${error.message}`);
        }
      }
      ...
      if (profileError) {
        errors.push(`perfil: ${profileError.message}`);
      }

      if (errors.length > 0) {
        toast({
          title: 'Error al resetear datos',
          description: `No se pudieron eliminar algunos datos: ${errors.join('; ')}`,
          variant: 'destructive',
        });
      ...
      toast({
        title: 'Error',
        description: 'Ocurrió un error al resetear los datos. Intenta de nuevo.',
        variant: 'destructive',
      });
      return { error: err };
    } finally {
  ...
  const resetOperationalData = async () => {
    ...
    try {
      ...
      if (errors.length > 0) {
        toast({
          title: 'Error al resetear datos operativos',
      ...
      toast({
        title: 'Error',
        description: 'Ocurrió un error al resetear los datos operativos. Intenta de nuevo.',
        variant: 'destructive',
      });
      return { error: err };
    } finally {
  ...
  // Single source of load: fetchData() fetches transactions + budgets + categories. No separate effect to avoid double fetch.
    ...
    try {
      ...
      if (categoriesRes.error) {
        ...
        if (categoriesToUpdate.length > 0) {
          // Persist color fixes to DB; await so setCategories runs after DB is updated
          await Promise.all(categoriesToUpdate.map(update =>
      ...
      toast({
        ...
        description: 'No se pudieron cargar los datos. Revisa tu conexión e intenta de nuevo.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const addBudget = useCallback(async (budget: Omit<Budget, 'id'>) => {
    ...
    // Use upsert to prevent duplicates for same category+month (DB unique: user_id, category, month)
    const { data, error } = await supabase
      .from('budgets')
      .upsert({
        user_id: user.id,
  ...
  // Exclude transfers between own accounts so net flow is not inflated
  ...
  // Filtered summary - respects date filters; exclude transfers for net flow consistency
  ...
  return useMemo(() => ({
    ...
    transactions, allTransactions, budgetsWithSpending, paymentMethods, loading, summary, filteredSummary,
    expensesByCategory, insights, yieldStatistics, currency, decimalPlaces, onboardingDecision, hasPendingImport,
    welcomeCompleted, importProgress, pendingImportData, addTransaction, addTransactionsBulk, deleteTransaction,
    addPaymentMethod, updatePaymentMethod, deletePaymentMethod, addTransfer, updateProfile, convertCurrency,
    setOnboardingDecisionFn, highlightedCard, setHighlightedCard, confirmPendingImport, startImport, cancelImport,
    confirmImportData, addBudget, deleteBudget, fetchData, categories, addCategory, updateCategory, deleteCategory,
    updateTransaction, orphanedTransactions, dateFilter, sortConfig, setSortConfig, loadMore, updateFilter, hasMore,
    rangeTransactions, totalTransactionsCount, resetProfileData, resetOperationalData, lastUpdated, updateCategoryGoal,
    baseColor, themeVars,
  ]);
}
